...
import { checkStatus, SUCCESS, TOKEN_FAIL, USER_FAIL } from '@/api/helper'
...
const config = {
  ...
  // 设置超时时间，默认超时时间60s
  timeout: import.meta.env.VITE_APP_HTTPS_TIMEOUT || 60 * 1000
  // 跨域时允许携带凭证
  ...
  constructor(config: AxiosRequestConfig) {
    ...
    this.service.interceptors.response.use(
      response => {
        ...
        // 登录失效
        if (data.code === TOKEN_FAIL || data.code === USER_FAIL) {
          userStore.clear()
          authStore.clear()
          // 关闭socket
          socketStore.close()
